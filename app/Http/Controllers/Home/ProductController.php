...
class ProductController extends Controller
{
    public function index(Request $request ,$slug){
        $data=array();$breadcrumbTitle='Product';
        ...
        if(isset($category) && $category->count()>0){
            ...
            if(isset($request->type) && $request->type === 'discount'){
                $data = Product::where(['category_id'=>$category->id,'status'=>1])->whereNotNull('discount')->paginate(26);
            }else{
                $data = Product::where(['category_id'=>$category->id,'status'=>1])->paginate(26);
            }
        }else if(isset($brand) && $brand->count()>0){
            $breadcrumbTitle = $brand->title;
            if(isset($request->type) && $request->type === 'discount'){
                $data = Product::where(['brand'=>$brand->id,'status'=>1])->whereNotNull('discount')->paginate(26);
            }else{
                $data = Product::where(['brand'=>$brand->id,'status'=>1])->paginate(26);
            }
        }else if(isset($product) && $product->count()>0){
            return $this->productDetail($slug);
        }else if($slug === "mall-rack"){
    ...
    public function presentationList(Request $request ,$slug){
        ...
        if(isset($category) && $category->count()>0){
            $data = Product::where('category_id',$category->id)->where('presentation',1)->paginate(26);
        }else if(isset($brand) && $brand->count()>0){
            $data = Product::where('brand',$brand->id)->where('presentation',1)->paginate(26);
        }else if(isset($product) && $product->count()>0){
